...
methods:{
  get_opts(val,update){
    ...
    update(()=>{
      ...
      var opts={method:"GET",url:"/api/product/search?keyword="+encodeURIComponent(val)};
      request(opts,this.serviceName).then(resp=>{
        ...
        for(var c of resp.data.list){
          opts.push({value:c.id,label:c.name,price:c.price});
        }
        this.opts=opts;
      });
    });
...
computed:{
  value:{
    ...
    set(v){
      this.$emit('update:modelValue',{id:v.value,name:v.label,price:v.price});
    }
  }
},
template:`
<q-select v-model="value" :label="label" :options="opts"
  ...
  <template v-slot:prepend>
   <q-icon name="inventory_2" color="blue" size="1em"></q-icon>
  </template>
