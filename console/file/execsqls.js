...
methods:{
execute() {
    var opts={method:"POST", private:false,
      headers:{access_token:this.service.token},
      url:"/api/sqlexec",
      data:{service:this.serviceName, db:this.db, sql:this.sql, cid:this.cid}};
    request(opts, this.service.name).then(resp => {
        ...
        this.hint=this.tags.useTime + resp.data.time + 'ms';
    });
}
},
...
<q-layout view="lHh lpr lFf" container style="height:100vh;">
  <q-header class="bg-grey-1 text-primary">
    <q-toolbar>
      ...
      <q-toolbar-title>{{tags.execsqls}}</q-toolbar-title>
    </q-toolbar>
<q-list class="q-pa-sm" dense>
  <q-item>
   <q-item-section>
    <q-select v-model="serviceName" :options="services" :label="tags.service" outlined dense></q-select>
   </q-item-section>
   <q-item-section>
    <q-input v-model="db" :label="tags.db" outlined dense></q-input>
   </q-item-section>
   <q-item-section>
    <q-input v-model="cid" :label="tags.cid" outlined dense></q-input>
   </q-item-section>
  </q-item>
  <q-item>
    <q-item-section>
