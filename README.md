...
提供每日数据远程加密备份，使得即使在地震、火灾等极端情况下，也只会最多丢失一天的数据；
在保证易用性的前提下，对数据存储、传输中的安全性做了周密的设计；
业务中可以对数据库敏感字段的加密，比如电话号码，并且密钥会定期自动更换，不需要用户做任何额外工作。

更多信息，请访问 http://www.zhijian.net.cn
网站中有服务端、客户端的使用指南与服务开发指导。

#### 软件架构
本系列软件服务侧技术用网格化思路实现，服务间通讯基于https协议，端云之间使用https协议，将可靠性、安全性内置于底层。
端侧采用hybrid思路，用网页开发交互界面，简化开发难度；在安装时界面代码已下载至本地，使用体验与原生实现并无差异。
...
端侧安装：
下载端侧应用程序安装即可，当前已提供安卓、Windows版本端侧，不同操作系统的端侧使用方法完全相同。
端侧接入分成两步：
1）在“设置”中添加公司（公司id、公司接入码），此步骤类似于连接wifi，一个终端可以接入多家公司；
2）点击右上角“登录”按钮输入公司分配的帐号、密码，登入公司系统，此步骤类似接入wifi后登录qq等软件系统，此后可以操作公司的业务软件。
